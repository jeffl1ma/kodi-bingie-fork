...
class DataType:
    ...
    def sync_func(self):
        ...
        with TimerFunc(f'Sync: {self.method} {self.item_type}', inline=True, log_threshold=0.001):
            return self.get_response_sync('sync', self.method, f'{self.item_type}s', limit=TRAKT_MAX_ITEMS_PER_PAGE, **self.sync_kwgs)
...
class SyncHiddenProgressWatched(DataType):
    ...
    def sync_func(self):
        ...
        with TimerFunc(f'Sync: {self.method} {self.item_type}', inline=True, log_threshold=0.001):
            response = []
            response += self.get_response_sync('users', self.method, limit=TRAKT_MAX_ITEMS_PER_PAGE, type=f'{self.item_type}s') or []
            return response
...
class SyncAllNextEpisodes(DataTypeEpisodes):
    ...
    def get_next_episodes_response(self, trakt_id):
        if not trakt_id:
            return
        return self.get_response_sync('shows', trakt_id, 'progress/watched', limit=TRAKT_MAX_ITEMS_PER_PAGE)
