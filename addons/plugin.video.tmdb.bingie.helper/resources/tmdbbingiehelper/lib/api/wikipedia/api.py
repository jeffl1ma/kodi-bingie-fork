...
USER_AGENT = f'TMDbBingieHelper/1.0.3 (https://github.com/matke-84/plugin.video.tmdb.bingie.helper matke@kodi.tv) {xbmc.getUserAgent()}'
...
WIKI_CCBYSA_IMG = 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e5/CC_BY-SA_icon.svg/330px-CC_BY-SA_icon.svg.png'
...
class WikipediaAPI(RequestWikiAPI):
    def __init__(self, language=None):
        if not language or language not in self.wikipedia_languages:
            language = get_setting('wikipedia_language', 'str') or DEFAULT_WIKI_LANGUAGE
...
class WikipediaGUI(xbmcgui.WindowXMLDialog):
    ...
    def set_backdrop(self):
        if not self.gui_meta.backdrop:
            return
        xbmcgui.Window(10000).setProperty('TMDbBingieHelper.Wikipedia.Backdrop', self.gui_meta.backdrop)
    ...
    def do_close(self):
        ...
        if not self.history:
            self.gui_meta.close()
            return self.close()
        self.do_setup(self.history.pop())
        self.do_init()
...
def do_wikipedia_gui(wikipedia, tmdb_type=None, xml_file=None, language=None, **kwargs):
    ...
    ui = WikipediaGUI(
        xml_file or 'script-tmdbbingiehelper-wikipedia.xml', ADDONPATH, 'default', '1080i',
...
def do_wikipedia_language(**kwargs):
    ...
    if x == -1:
        return
    set_setting('wikipedia_language', languages[x][0], 'str')
    return
